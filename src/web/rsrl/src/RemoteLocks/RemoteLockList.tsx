...
import { RemoteLock, RemoteLockOperation, RemoteLockModalData, AccessCardSelectModel } from "./remoteLockTypes";
...
export default function RemoteLockList(props: RemoteLockListProps) {
  ...
  const [accessCardsInfos, setAccessCardsInfos] = useState<AccessCardSelectModel[]>();
  ...
  const [modalOperation, setModalOperation] = useState(RemoteLockOperation.None);
  ...
  const onModalConfirm = (modifiedData: RemoteLockModalData | undefined, operation: RemoteLockOperation) => {
    if (modifiedData) {
      switch (operation) {
        case RemoteLockOperation.Add:
        ...
        <div className={props.className}>
          {remoteLocks ? (
            <div className="ui-list-flex-container">
              <RemoteLockSideMenu onClick={onSideMenuClick} isItemSelected={selected ? true : false} />
